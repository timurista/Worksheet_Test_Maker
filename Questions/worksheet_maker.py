...
class Worksheet():
	...
	def make_worksheet(self,**kwargs):
		...
		type = kwargs.get("type","Worksheet")
		qarray = [s.split("\t") for s in self.questions.split("\n")]
		print len(qarray)
		html_css = """
			<style> body {font-size: 11pt; } li{ margin-bottom: 10pt;} 
			li:nth-of-type(10n+10) { background-color: lightblue; page-break-after:always;} 
		...
		for question in qarray:
			if len(question)>1:
				...
				all_questions[section]+=""+LETTERS[0]+choices[0]+",&nbsp;"
				all_questions[section]+=""+LETTERS[1]+choices[1]+",&nbsp;"
				all_questions[section]+=""+LETTERS[2]+choices[2]+",&nbsp;"
